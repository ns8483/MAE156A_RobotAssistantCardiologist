...
//***** joystick values range from 0 to 1023 using 900 as right/up limit and 15 as left/down limit. can adjust later**********
int RULIM = 900;
int LDLIM = 15;
...
void loop(){
  if (radio.available()) {
    while(radio.available()) {
      ...
      }else if (joyPos[1] <= LDLIM){
        if (servoTopPos != 0){
          servoTopPos = servoTopPos - servoStepSize;
          servoTop.write(servoTopPos);
          Serial.println(servoTopPos);
        }
      }else if (joyPos[0] >= RULIM){
        if (servoBotPos != 180){
          servoTopPos = servoBotPos + servoStepSize;
          servoBot.write(servoBotPos);
          Serial.println(servoBotPos);
        }
      }else if (joyPos[0] <= LDLIM){
        if (servoBotPos != 0){
